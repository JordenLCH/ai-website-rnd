...
- **`theme.json`** — `{ name, tokens: {37 required CSS custom properties, plus optional}, sectionStyles: { <slug>: {layout, tone, vars?} } }`
...
**And the uploaded JSON is kept in hosting's database, not only on its disk** (2026-09-14). Each
publish writes a Payload **Website** row carrying `site.json`, `theme.json` and `org.json`; the
directory under `bundles/<domain>/` is a working copy the build restores from that row. This is
what makes the fleet-wide patch above real rather than theoretical: `npm run rebuild:bundles --
--deploy` in site-hosting re-renders every conversational site, which is how an improvement here
reaches sites that already exist. The photographs stay out of the database by decision — filenames
in the row, bytes on disk and in the deployed build — so a database-only restore returns
rebuildable sites with their pictures still missing.
...
`dev/fleet-archive/*`, `site-hosting/content/*` and the `mcp/fixtures` are **the pipeline's own
output from a POC** — not a specification, not a sample of real usage, and not a control group.
Seven themes and a handful of sites, all generated by the same model, are one distribution with
seven draws from it. "No current bundle does X" is a snapshot, not a guarantee, and it is usually
a statement about the generator's habits rather than about the world.

The failure mode is specific and it is seductive, because the archive is the only data lying
around. Three real instances, all in one session on 2026-09-14:

- Measured the eyebrow tracking across seven themes, found one outlier, and called it "hand-made"
  — inferring provenance from the result I wanted. `dev/` is gitignored; there was no evidence,
  and `docs/2026-09-04-spike-findings.md` records that theme's sibling as generated. There was no
  human control group at all.
- Found `Grid cols` of only 2 and 4 in the fleet and concluded a 6-column grid "isn't a real case".
  The schema allows 2–12 and the generator picks the number; it will emit 6, and when it did, the
  grid held six 111px columns at 1024px.
- Saw no bundle setting a token and concluded the default didn't matter.

**What to do instead.** Use the fleet for exactly two things: learning what the pipeline *currently
emits*, and finding defects in it. Decide what it *should* emit from how production sites are
really built, from published design-system contracts, and — the cheapest and most convincing — from
measuring real sites directly (`getComputedStyle` in a browser; see
`docs/research/2026-09-14-typographic-detail-in-art-direction.md` for the method and a worked
example). When output and real practice disagree, the default hypothesis is a pipeline defect.

**And assume the input gets stranger.** Real clients bring copy nobody designed for, and the model
composing their site is not predictable — it will reach for a column count, a tracking value or a
nesting depth no stored bundle contains. Anything the schema permits will eventually be generated,
so a layout must hold for the whole permitted range, not for the values the archive happens to
show. `renderer/src/blocks/*` `check()` rules and the CSS floors exist for exactly this;
`dev/fleet-archive` passing proves nothing about the next site.
